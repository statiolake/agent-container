...
function usage() {
  process.stderr.write(
    [
      ...
      "",
      "Run a configured agent-container task through the host broker.",
      "stdin is streamed to the task. Only tasks declared in settings.toml",
      "are available; this command cannot execute arbitrary commands.",
      "",
      "  KEY=VALUE       expose a named task argument as an environment variable",
      "  -- ARG ...      pass ordered positional arguments to the task's \"$@\"",
      "  --timeout-seconds N  set the task timeout",
      "",
    ].join("\n"),
  );
}
